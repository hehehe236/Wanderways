...
const rideSlice = createSlice({
    ...
    selectors:{
        ...
        selectRideAcceptedParcels: (state, rideId: number, parcelId: number) => state.find(ride => ride.rideId === rideId)?.acceptedParcelList?.find(parcel => parcel.parcelId === parcelId),
    },
    reducers: {},
        ...
        builder.addMatcher(
            ...
            (state, { payload }: PayloadAction<RideAcceptedParcels[]>) => {
                const updateRide = state.map(ride => {
                    ...
                    return ride.rideId === payload[0].rideId ? {...ride , acceptedParcelList: payload } : ride;
                })
                return updateRide
            }
        )
